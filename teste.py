...
import sqlite3   # <<< NOVO: para usar o SQLite
...
PASTA_RESULT = "imagens_pdf/result"  # padrão; pode ser mudada na UI
ARQUIVO_GABARITO = "gabarito.txt"
ARQUIVO_BOLHAS = "bolhas.txt"  # layout global

# >>> NOVO: JSON ÚNICO NA RAIZ DO PROJETO <<<
...
# Overlays
DESENHAR_GRADE_TODAS_BOLHAS = True         # desenha todas as bolhas por cima
COR_GRADE = (0, 200, 255)                  # BGR
ESPESSURA_GRADE = 1
...
def auto_reposicionar_bolhas(gray_area, bolhas_esq, bolhas_dir):
    try:
        ...
        cc, warp = cv2.findTransformECC(
            ref, tgt, warp, motionType=cv2.MOTION_AFFINE,
...
# =============================
# HELPERS NOVOS (NOME POR QR)
# =============================
def _gerar_caminho_unico_por_qr(codigo_qr, pasta_saida, ext="png"):
    """
    Gera um caminho único: <pasta>/<QR>.png
    Se já existir, cria <QR>_2.png, <QR>_3.png, ...
    """
    base = f"{codigo_qr}.{ext}"
    caminho = os.path.join(pasta_saida, base)
    if not os.path.exists(caminho):
        return caminho, base
    i = 2
    while True:
        base_i = f"{codigo_qr}_{i}.{ext}"
        caminho_i = os.path.join(pasta_saida, base_i)
        if not os.path.exists(caminho_i):
            return caminho_i, base_i
        i += 1
...
# =============================
# PROCESSAMENTO DE UM CARTÃO
# =============================
def processar_cartao(caminho_imagem, gabarito, pasta_saida):
    """
    Processa a imagem, corrige, desenha e salva PNG final usando o CÓDIGO DO QR como nome.
    Retorna: (caminho_png_final, (chave_qr, entry_json_no_formato_novo))
    """
    ...
    # Auto-reposicionamento (ECC/AFFINE)
    if usar_auto:
    ...
    for g in range(0, 50, 5):
        ...
        logs.append(("esq", (g // 5) + 1, decision, reason))
    ...
    for g in range(0, 50, 5):
        ...
        logs.append(("dir", (g // 5) + 11, decision, reason))

    respostas_final = respostas_esq + respostas_dir  # 20

    # Comparação e desenho
    acertos = anuladas = em_branco = 0
    ...
    # Dados por questão no formato novo
    questoes_json = {}
    for i, resp_aluno in enumerate(respostas_final):
        correta = gabarito[i] if i < len(gabarito) else None
        ...
        # destacar a resposta marcada quando houver (visual)
        if marcada in ['A', 'B', 'C', 'D', 'E']:
            cor = (0, 255, 0) if status == "acerto" else (0, 0, 255)
            if i < 10:
                grupo_idx = i
    ...
    erros = total_questoes - acertos - anuladas - em_branco

    # recombina área processada
    ...
    resumo = [
        f"Acertos: {acertos}/{total_questoes}",
        f"Anuladas: {anuladas}",
        f"Em branco: {em_branco}",
    ...
    # salvar com nome do QR Code
    if codigo_qr:
        caminho_saida, nome_arquivo = _gerar_caminho_unico_por_qr(codigo_qr, pasta_saida, ext="png")
    else:
        base_fallback = os.path.splitext(os.path.basename(caminho_imagem))[0]
        nome_arquivo = f"{base_fallback}.png"
        caminho_saida = os.path.join(pasta_saida, nome_arquivo)

    cv2.imwrite(caminho_saida, imagem_final)

    # log opcional (preservado)
    base_log = os.path.splitext(nome_arquivo)[0]
    resultado_txt = os.path.join(pasta_saida, f"{base_log}_resultado.txt")
    with open(resultado_txt, "w", encoding="utf-8") as f:
        f.write(f"Arquivo origem: {caminho_imagem}\n")
        f.write(f"PNG final: {nome_arquivo}\n")
        f.write(f"Resumo: {resumo}\n\n")
        for lado, qnum, decision, reason in logs:
    ...
    entry = {
        ...
        "arquivo": nome_arquivo,          # nome do PNG salvo (por QR)
        "timestamp": timestamp
    }

    chave = codigo_qr if codigo_qr else os.path.splitext(os.path.basename(caminho_imagem))[0]
    return caminho_saida, (str(chave), entry)

# =============================
# PROCESSAMENTO EM LOTE + JSON
# =============================
def processar_todas_as_imagens(gabarito, pasta_saida, arquivo_json_path, progress_cb=None):
    """
    Processa todas as imagens em PASTA_TEMP, salva PNG (nome = QR) e
    escreve/atualiza o JSON único no caminho indicado.
    """
    imagens_corrigidas = []
    os.makedirs(pasta_saida, exist_ok=True)
    ...
    # carrega JSON existente (se houver), para acumular
    if os.path.exists(arquivo_json_path):
        try:
            with open(arquivo_json_path, "r", encoding="utf-8") as jf:
    ...
    for idx, arquivo in enumerate(arquivos, start=1):
        ...
        caminho_saida, result_pair = processar_cartao(caminho_img, gabarito, pasta_saida)
        if caminho_saida:
            imagens_corrigidas.append(caminho_saida)
        if result_pair:
            chave, entry = result_pair
            # >>> AQUI: ADICIONA/SOBRESCREVE POR CÓDIGO <<<
            resultados_geral[str(chave)] = entry

            # salva o JSON a cada cartão processado (robusto para travamentos)
            try:
                with open(arquivo_json_path, "w", encoding="utf-8") as jf:
...
def limpar_pastas():
    ...
    # >>> NOVO: limpar também o JSON único da raiz
...
def limpar_banco_de_dados_tabela_cartao_resposta(parent=None):
    try:
        conn = sqlite3.connect("database")  # nome fixo do arquivo do seu banco
        cur = conn.cursor()
        cur.execute("DELETE FROM CartaoResposta")
        conn.commit()
        conn.close()
        QMessageBox.information(parent, "Sucesso", "Tabela CartaoResposta limpa com sucesso!")

        return True
    
    except Exception as e:
        print("Falha ao limpar tabela CartaoResposta:", e)
        QMessageBox.critical(parent, "Erro", f"Falha ao limpar tabela CartaoResposta:\n{e}")
        return False

def salvar_resultados_no_banco(parent=None):
    """
    Lê o arquivo Resultados.json e salva/atualiza na tabela CartaoResposta do SQLite.
    """
    if not os.path.exists(ARQUIVO_JSON):
        QMessageBox.warning(parent, "Erro", "Nenhum arquivo de resultados (Resultados.json) encontrado.")
        return
    ...
    try:
        ...
        QMessageBox.critical(parent, "Erro", f"Falha ao abrir Resultados.json:\n{e}")
        return
    ...
    try:
        ...
        for codigo, entry in resultados.items():
            # procura id_aluno
            cur.execute("SELECT id_aluno FROM Aluno WHERE codigo = ?", (codigo,))
            row = cur.fetchone()
            if not row:
                continue  # ignora se o aluno não estiver cadastrado
            id_aluno = row[0]

            totais = entry.get("totais", {})
            acertos = totais.get("acertos", 0)
            erros = totais.get("erros", 0)
            anuladas = totais.get("anuladas", 0)
            em_branco = totais.get("em_branco", 0)
            total_questoes = totais.get("total_questoes", 0)
            resultado_str = json.dumps(entry, ensure_ascii=False)

            cur.execute("""
                INSERT INTO CartaoResposta (codigo, resultado, id_aluno, acertos, erros, anuladas, em_branco, total_questoes)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ...
            """, (codigo, resultado_str, id_aluno, acertos, erros, anuladas, em_branco, total_questoes))

        conn.commit()
        conn.close()
        QMessageBox.information(parent, "Sucesso", "Resultados salvos no banco de dados com sucesso!")

    except Exception as e:
        QMessageBox.critical(parent, "Erro", f"Falha ao salvar no banco:\n{e}")
# =============================
...
class BubbleEditorDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Reposicionar Bolhas (página por página)")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.pasta_saida = PASTA_RESULT  # pasta de saída configurável (para PNG/PDF)

        self.setWindowTitle("Sistema de Correção de Cartões de Resposta")
        self.setGeometry(200, 200, 720, 640)

        layout = QVBoxLayout()
        self.label = QLabel("Fluxo de trabalho:")
        layout.addWidget(self.label)

        # Linha da pasta de saída
        self.lbl_pasta_saida = QLabel(f"Pasta de saída atual: {self.pasta_saida}")
        layout.addWidget(self.lbl_pasta_saida)

        btn_pick_out = QPushButton("Escolher Pasta de Saída (RESULT)")
        btn_pick_out.clicked.connect(self.escolher_pasta_saida)
        layout.addWidget(btn_pick_out)
        ...
        layout.addWidget(btn4)
          # >>> NOVO BOTÃO <<<
        btn_db = QPushButton("6. Salvar no Banco de Dados (SQLite)")
        btn_db.clicked.connect(lambda: salvar_resultados_no_banco(self))
        layout.addWidget(btn_db)

        btn_remove_db = QPushButton("Limpar Tabela CartaoResposta (SQLite)")
        btn_remove_db.clicked.connect(lambda: limpar_banco_de_dados_tabela_cartao_resposta(self))
        ...
        btn7 = QPushButton("Limpar TEMP/RESULT (padrão)")
    ...
    # ====== escolher pasta de saída (PNG/PDF) ======
    def escolher_pasta_saida(self):
        path = QFileDialog.getExistingDirectory(self, "Escolher Pasta de Saída")
        if path:
            self.pasta_saida = path
            self.lbl_pasta_saida.setText(f"Pasta de saída atual: {self.pasta_saida}")
            QMessageBox.information(
                self,
                "Pasta de Saída",
                f"Arquivos (PNGs e PDF) serão salvos em:\n{self.pasta_saida}\n"
                f"O arquivo JSON único fica sempre na raiz: {ARQUIVO_JSON}"
            )
    ...
    def processar_imagens(self):
        ...
        def worker(cb, cancelled):
            def progress_cb(i, total):
                ...
                cb(i, total)
            # >>> JSON ÚNICO NA RAIZ <<<
            arquivo_json_path = ARQUIVO_JSON
            imgs = processar_todas_as_imagens(self.gabarito, self.pasta_saida, arquivo_json_path, progress_cb)
        ...
        if self.imagens_corrigidas:
            QMessageBox.information(
                self,
                "OK",
                f"{len(self.imagens_corrigidas)} imagens processadas.\nJSON único salvo/atualizado em:\n{ARQUIVO_JSON}"
            )
    ...
    def limpar(self):
        ...
        QMessageBox.information(
            self,
            "Limpeza",
            "TEMP e RESULT (padrão) foram apagadas, e o JSON único (Resultados.json) na raiz foi removido!\n"
            "Obs.: se você escolheu uma pasta de saída personalizada, ela NÃO é apagada por aqui."
        )
