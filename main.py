...
# OBS: O JSON agora é definido dinamicamente pela pasta de saída escolhida
...
def processar_cartao(caminho_imagem, gabarito, pasta_saida):
    ...
    # >>> NOVO: salvar com nome do QR Code <<<
    if codigo_qr:
        caminho_saida, nome_arquivo = _gerar_caminho_unico_por_qr(codigo_qr, pasta_saida, ext="png")
    else:
        # fallback: usa o base do arquivo de entrada, mas mantendo sua exigência quando possível
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.pasta_saida = PASTA_RESULT  # >>> NOVO: pasta de saída configurável
    ...
    def escolher_pasta_saida(self):
        ...
        if path:
            ...
            QMessageBox.information(self, "Pasta de Saída", f"Arquivos (PNGs e JSON) serão salvos em:\n{self.pasta_saida}")
        ...
        def worker(cb, cancelled):
            def progress_cb(i, total):
                if cancelled["flag"]:
                    return
                cb(i, total)
            arquivo_json_path = os.path.join(self.pasta_saida, "resultados.json")
    ...
    def limpar(self):
        ...
        QMessageBox.information(
            self,
            "Limpeza",
            "TEMP e RESULT (padrão) foram apagadas!\n"
